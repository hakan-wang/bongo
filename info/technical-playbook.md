# Bongo — Technical Playbook (for Tech 1 & Tech 2)
...
> **Golden rule from §6:** Bongo checks a step **against reality, not against another AI's opinion.**
...
**"Verification is cheaper than generation."** Bongo runs a *cheap* model, watches each step,
...
## 1. How Bongo sits on top of provider APIs (Tech 2's surface)

**Plain English:** the customer changes one line — points `base_url` at Bongo, keeps their own
key — and Bongo proxies to the real provider while watching every step.
...
| **C. OTel auto-instrumentation** | If the customer app uses a framework (LangChain/LlamaIndex/CrewAI), one-line OpenInference or OpenLLMetry instrumentor emits spans; Bongo receives OTLP | Second integration path, post-demo | Low (their side) |
...
- **Multi-step structure = a span tree under one `trace_id`**: `invoke_agent` (parent) → child
  ...
  `session.id` / `conversation.id` so turns group correctly. This is how Bongo reconstructs
...
is exactly why Bongo verifies **per-step, incrementally**, not once at the end. (Bonus, verified:
...
- **Routing (RouteLLM-style):** pick weak vs strong **before** generation. ~95% of GPT-4 quality
  ...
  it's the loser's framing. Bongo's edge is **per-step runtime correction**, which routing/cascades
...
long-term moat. Because Bongo proxies real traffic, it measures reliability on the task the
...
Bongo is a proxy (extend `proxy.py`, optionally LiteLLM underneath) that emits OTel GenAI spans,
