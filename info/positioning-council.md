...
1. **Lead sentence:** *"Assay is the independent reliability layer that catches and fixes an AI
...
> ### "Assay is the independent reliability layer that catches and fixes an AI agent's bad steps in real time — so a cheap model runs as reliably as an expensive one, on any provider."
...
> *"Gateways route and retry on errors; observability tells you after the fact; Assay watches
...
> *"Assay is the independent reliability/QA layer for AI agents."*
...
- **Vendor-neutral / cross-provider** = the moat. A model lab only ever sharpens *its own* model;
  it will never tell you "use the competitor's cheaper one." Assay sits above all of them.
...
- [ ] Use cost as the **proof** slide: "Flash + Assay matches Pro at ~Nx less," cost-per-completed-task.
