...
|------|---------------|----------------|
| **Cheap verifiability** | Can Bongo tell a step is wrong *deterministically, without knowing the answer in advance*? | This is the whole technical bet (`SOURCE-OF-TRUTH.md` §6). If the check needs an LLM judge, the demo's credibility breaks. Non-negotiable. |
...
| **Wow** | Does the cheap model *visibly* fail, and does Bongo *visibly* catch and fix it live, with a real cost gap? | The stage moment is "watch it break, watch Bongo save it." |
...
- **The cost gap is the cleanest story.** "Flash wrote code that failed the test. Bongo
...
A small multi-step coding agent runs on a **cheap** model. Bongo sits in front of it (one
...
> plus return it as a passing solution." We ship a fixed hidden test suite Bongo runs.
...
**Run B — cheap model + Bongo (the save), live:**
1. **TRACE** — Bongo captures the generate step (input, output, model, tokens, cost).
2. **VERIFY** — Bongo runs the unit tests against the generated code. Deterministic. The
   test fails → Bongo *knows the step is bad*, without an LLM judge, without knowing the
...
3. **INTERVENE — step 1, retry cheap with guidance:** Bongo re-runs the *same cheap model*,
...
4. **INTERVENE — step 2, escalate just this step (if retry still red):** Bongo falls back to
   the **strong model for this one step only** — not the whole workflow. Tests go **green.**
5. **LEARN** — Bongo logs the failure + fix to the reliability database (the moat).
...
- **Cheap + Bongo:** **tests pass** AND cost ≈ cheap (a couple of cheap units, with at most
...
- **Owner handoff:** Tech 1 (engine) wires the code sandbox + test runner into the
  ...
  "watch it break, watch Bongo save it" beat.
