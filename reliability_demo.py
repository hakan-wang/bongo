"""
Assay reliability demo:  python3 reliability_demo.py
...
Assay catches every bad step and recovers it, so the final result is 100% correct,
...
def main():
    print("\nASSAY RELIABILITY  ·  cheap model, watched and corrected\n" + "-"*60)
    total_cost = 0
    correct = 0
    ...
    for task in INPUTS:
        ...
        # how the cheap model would have done with NO Assay (first attempt only)
        first = t["attempts"][0]
    ...
    print("\n" + "-"*60)
    print(f"Correctness WITH Assay:        {correct}/{len(INPUTS)}  ({correct/len(INPUTS)*100:.0f}%)")
    print(f"Correctness cheap model alone: {cheap_alone_correct}/{len(INPUTS)}  ({cheap_alone_correct/len(INPUTS)*100:.0f}%)")
    print(f"\nCost WITH Assay:        {total_cost} units")
