...
Assay sits between an app and the LLM API, watches every step of a multi-step agent, catches the cheap model's bad steps, and corrects them. Result: a cheap model performs like an expensive one. You stop overpaying for reliability you can get another way.

## What Assay is
A drop-in layer (one line: change the base_url, keep your own key). It is:
1. **Independent** of any model vendor. It works across Gemini, OpenAI, Anthropic, open models. A model maker will only ever make its own model better. Assay sits above all of them. (This is also the answer to "why won't a big LLM company build it" — they would only do it for their own agent. Confirmed by a QRT advisor.)
...
For a multi-step agent workflow, Assay traces each step, detects when a step went wrong, and helps it recover (retry with guidance, supply missing info, or fall back to a stronger model just for that step). The point: make a cheap, error-prone model trustworthy enough to use in production.

## Side feature: cost + comparison
Because Assay sees every step and its quality, it can:
- Tell you where you are overpaying (e.g. "you are using Pro where Flash + Assay would do").
...
"Watch every step and catch the bad ones" only works if Assay can tell a step is wrong. How it knows depends on the domain:
...
Brolly is a multi-step agentic workflow on the Gemini API: analyze a raw video, reason about the best clips, then cut and assemble. Assay would:
...
- Let Håkan run cheaper Gemini Flash instead of Pro, because Assay backstops Flash's mistakes.
- Tell him when Pro is overkill and Flash + Assay is enough.

## Who needs it
Any startup or team that put an LLM API inside their product, especially with multi-step agent workflows (analyze, reason, act). They all face the same tradeoff: cheap-and-unreliable vs expensive-and-safe. Assay removes the tradeoff.

## Objections, answered
- **Won't a big LLM company build it?** Only for their own model. Assay is independent and cross-model. (QRT advisor confirmed this is exactly why he would use it.)
...
- **Crowded (Coval, Chronicle, Respan, Arga, Roark)?** Those are dev tools to test your OWN agent before shipping. Assay is a runtime layer that catches and corrects in production, across models, and saves cost. Different lane.
