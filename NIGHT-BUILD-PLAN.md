# NIGHT-BUILD-PLAN.md — Ship Verity by 13:00 Paris (Sun Jun 28)

**Scope line (24h deadline):** Re-skin the existing engine onto the finance money-shot, make the recovery ladder + honesty case VISIBLE, rename to Verity everywhere, cut the <=5 min video. **Zero new core engine on the critical path.** Reuse `demo/scenarios.py`, `demo/server.py`, `index.html`, `demo/gateway.py`, `demo/real_proof.py`.

---

## P0 — Must ship (the video cannot exist without these)

### 1. Rename to Verity [~20 min] [HARD DEPENDENCY for video + deck]
- [ ] 1.1 Wordmark on `index.html` header (swap "Bongo")
- [ ] 1.2 Slide 1 title + logo
- [ ] 1.3 Find/replace "Bongo" across README, dashboard, deck
- [ ] 1.4 10-min trademark clearance check (USPTO/EUIPO software class); if crowded → **Plumbline**
- [ ] 1.5 VERIFY: grep repo for "Bongo" returns only changelog/history
...
- [ ] 4.2 Verity flags **"can't deterministically verify — low-confidence, NOT auto-passed"**
...
- [ ] 6.3 Cost-per-COMPLETED-task line (not per-token); scoreboard cheap 67% → Verity 100%, ~62% cheaper — as RECEIPTS at the end only
...
- [ ] 8.3 Public GitHub repo, Verity name, screenshot of the catch
...
- [ ] 10. **Reliability-per-dollar leaderboard** [~45 min]: each provider row = accuracy %, $, composite score; Verity row on top beats every model on BOTH axes (reuse scoreboard data)
...
3. grep for "Bongo" — none in shipped assets
